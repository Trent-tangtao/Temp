...
from net.SEresnet import SEResNeXt
...
def train(args):
    ...
    # 224 224
    #model = VGG_16(width=norm_size, height=norm_size, depth=3, classes=CLASS_NUM)

    # 32 32
    # model = LeNet.build(width=norm_size, height=norm_size, depth=3, classes=CLASS_NUM)

    # 224 224
    # model = unet(width=norm_size, height=norm_size, depth=3, classes=CLASS_NUM)

    # 224 224 Input Shape is  3 x 224 x 224 (th)
    model = ResnetBuilder.build_resnet_18((3, norm_size, norm_size), CLASS_NUM)

    # 299 299 Input Shape is 299 x 299 x 3 (th) or 3 x 299 x 299 (th)
    # model = inception_v4(CLASS_NUM, 0.5, None, True)

    # size size 3
    # model = SEResNeXt(norm_size, CLASS_NUM).model
